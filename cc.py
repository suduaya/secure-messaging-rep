...
def verify(pem, data, sign):
    try:
        obj = OpenSSL.crypto.load_certificate(OpenSSL.crypto.FILETYPE_PEM, pem) #cert load
        print "verify"
        return (OpenSSL.crypto.verify(obj, sign, data, "sha256") is None) or (OpenSSL.crypto.verify(pem, sign, data, "sha1") is None)# Sha256, assumindo que None = True, else False
    except Exception, e:
        return False

mode = "AUTHENTICATION"
cert, session = certificate(mode)
signature = sign("ola", session, mode)
print base64.b64encode(signature)
print verify(pem = cert, data ="ola", sign =signature)
